...
func (r *TransportRepository) GetByID(ctx context.Context, id int64) (*entity.Transport, error) {
	...
	query := `SELECT * FROM transport WHERE id = $1`
...
func (r *TransportRepository) ListAll(ctx context.Context, count, start int) ([]entity.Transport, error) {
	...
	query := `SELECT * FROM transports LIMIT $1 OFFSET $2`
...
func (r *TransportRepository) ListByType(ctx context.Context, transportType string, count, start int) ([]entity.Transport, error) {
	...
	transports := make([]entity.Transport, 0, 100)
	query := `SELECT * FROM transports WHERE transport_type = $1 LIMIT $2 OFFSET $3`
	rows, err := r.QueryxContext(ctx, query, transportType, count, start)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
